...
- **Upgrading from version 14 to 25** (this page)
...
The *destinationappsformigration* setting is a configuration setting on the Business Central Server. In short, it's used to transfer ownership of the existing tables to the table migration extension. For more information, see [DestinationAppsForMigration](https://learn.microsoft.com/en-us/dynamics365/business-central/dev-itpro/upgrade/upgrade-destinationappsformigration).

- **migration.json** file
The *migration.json* file is used to migrate tables and fields from one extension to another. In this case, migration is from the table migration extension to system and base application tables. For more information about the migration.json, see [The Migration.json File](https://learn.microsoft.com/en-us/dynamics365/business-central/dev-itpro/developer/devenv-migration-json-file).
...
!!! output ""
    <font color="lime">Configuration loaded on C:\Upgrade\config.json.</font>  
    <font color="orange">WARNING: Please open Readme.txt file in C:\Upgrade\License.</font>  
    ...
    <font color="orange">WARNING: Do not forget to copy all the C/AL to AL converted files to this folder before compiling the app.</font>  
    ...
    <font color="orange">WARNING: If there's additional custom apps, other than the ones on the configFile.json file, don't forget to add the app id to the migration.json file on the ls-upgrade-2nd_version-migration (either editing the file or using the Add-CustomExtension cmdlet).</font>  
...
!!! note
    It is possible to rename custom tables if needed. To do that, include the `-CustomTablesMappingFilePath` flag in the Strip-BC14CALObjects.  
    Example:
    ```
...
!!! note
    No breaking changes should exist so no errors should result be shown.  
...
!!! warning
    ...
    Don't use the .app file you get on VSCode after downloading the symbols. These files contains only the symbols and are lacking the source code and won't work properly. For more information check **Error 8** in the **Troubleshooting** section.
...
!!! tip
    ...
    Synchronize the tenant by running the following line on task 11:
    ```powershell
...
!!! note
    ...
    another app.  
    Table **Facebook Pages Groups** :: The table 'Facebook Pages Groups' cannot be located. Removing tables is not allowed unless they are temporary or are being moved by migration to another app.  
...
!!! warning    
    If no other errors are presented, you can run the `Sync-NAVApp` command with the `ForceSync` Mode flag.  
    Otherwise you should look at other errors and check if you can use `ForceSync`.  
    Remember that the data on the table can be deleted when forcing the synchronization.
